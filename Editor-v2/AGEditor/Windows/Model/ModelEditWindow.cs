...
namespace AGEditor
{
    public partial class ModelEditWindow : Form
    {
        private Model2D _model;
        private ModelFrameEditPanel _framePanel;
        private ModelFrameEditWrapper _frameEditWrapper;

        public ModelEditWindow()
        {
            InitializeComponent();

            _framePanel = new ModelFrameEditPanel();

            _frameEditWrapper = new ModelFrameEditWrapper(_framePanel);
            _frameEditWrapper.Dock = DockStyle.Fill;
            this.panel1.Controls.Add(_frameEditWrapper);

            checkedListBox1.ItemCheck += new ItemCheckEventHandler(checkedListBox1_ItemCheck);
        }

        public ModelEditWindow(Model2D model)
        {
            this._model = model;
            InitializeComponent();
        ...
        private void treeView1_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            {
                Model2D selModel = selNode.Parent.Tag as Model2D;
                Action2D selAction = selNode.Tag as Action2D;

                //
                checkedListBox1.Items.Clear();
                foreach (var action in selModel.Actions)
                {
                    checkedListBox1.Items.Add(action);
                }
            }
            else if (selNode.Tag.GetType() == typeof(Direction2D))
            {
                Action2D selAction = selNode.Parent.Tag as Action2D;
                Direction2D selDirection = selNode.Tag as Direction2D;

                checkedListBox1.Items.Clear();
                foreach (var direction in selAction.Directions)
                {
                    checkedListBox1.Items.Add(direction);
                }
            }
            else if (selNode.Tag.GetType() == typeof(Frame2D))
            {
                Direction2D selDirection = selNode.Parent.Tag as Direction2D;
                Frame2D selFrame = selNode.Tag as Frame2D;

                _framePanel.Settings(selDirection.Frames, selFrame);

                checkedListBox1.Items.Clear();
                foreach (var frame in selDirection.Frames)
                {
                    if (frame == selFrame)
                    {
                        checkedListBox1.Items.Add(frame, true);
                    }
                    else
                    {
                        checkedListBox1.Items.Add(frame, false);
                    }
                }
            }
        }
        ...
        {
            PreviewWindow dlg = new PreviewWindow(_model);
            dlg.ShowDialog();
            //TreeNode selNode = treeView1.SelectedNode;
            //if (selNode == null)
            //{
        ...
        {
            if (this._framePanel.Frames != null)
            {
                int index = e.Index;
                if (e.NewValue == CheckState.Unchecked)
                {
                    // unchecked;
                    this._framePanel.SetVisible(this._framePanel.Frames[index], false);
                }
                else if (e.NewValue == CheckState.Checked)
                {
                    // checked;
                    this._framePanel.SetVisible(this._framePanel.Frames[index], true);
                }
            }
        }
    }
